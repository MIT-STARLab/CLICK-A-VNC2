...
static size_t handle_block(void *block_ptr, size_t block_start, size_t block_size, uint8 **data, size_t *len)
{
...
{
    // Handle CCSDS header
    size_t remaining = handle_block(&packet->header, 0, UART_PACKET_HEADER_LEN, &data, &len);
    if (remaining == 0)
    {
    ...
    // Handle image blob
    remaining = handle_block(&packet->blob, UART_PACKET_HEADER_LEN, packet->blob_len, &data, &len);
    if (remaining > 0) return remaining;

    // Handle CRC
    remaining = handle_block(&packet->crc, UART_PACKET_HEADER_LEN + packet->blob_len, 2, &data, &len);
    if (remaining > 0) return remaining;
